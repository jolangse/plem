...
if __name__ == '__main__':
    ...
    if sensor_value < 0 or sensor_value > 255:
        print("Humidity values are in range 0 to 255")
        exit(1)
    ...
    #  5 bytes node ID
    #  1 byte 0x20 (datatype int16 temperature value)
    ...
    values = (
            ...
            32,          # Type: Data type, see SensorType.java
            1,          # Length: 1 byte
            int(round(sensor_value)), # Value (will be packed to 1 positive byte)
            )
